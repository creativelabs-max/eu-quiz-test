...
function openRapidFireQuestion() {
  ...
  if (jswap) {
    jswap.style.display = (gameMode === 'suddendeath' || gameMode === 'timeattack') && allowSwap ? 'inline-flex' : 'none';
...
function useSwapJoker() {
  if (gameMode !== 'suddendeath' && gameMode !== 'timeattack') return;
  if (!teams || !teams[activeTeam] || !teams[activeTeam].jokers) return;
