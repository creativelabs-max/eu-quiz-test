...
function selectCategory(catNum) {
  ...
  if (gameMode === 'timeattack') {
    ...
    if (typeof configureSetupScreenForCampaign === 'function') {
      configureSetupScreenForCampaign();
    } else {
      ...
      if (botToggleContainer) botToggleContainer.style.display = 'block';
    }
  }
}

function configureSetupScreenForCampaign() {
  const campaignSettings = document.getElementById('campaign-settings');
  if (campaignSettings) campaignSettings.style.display = 'block';
  ...
  const botEnable = document.getElementById('setup-enable-bot');
  if (botEnable) botEnable.checked = false;

  const botSettingsGroup = document.getElementById('setup-bot-settings-group');
  if (botSettingsGroup) botSettingsGroup.style.display = 'none';
