...
function toggleTheme() { 
  ...
  try {
    safeLocalStorage.setItem('eu_quiz_theme', isDark ? 'dark' : 'light');
  } catch(e) {}
  ...
  const mapImg = document.querySelector('.fullscreen-map-img');
  if (mapImg) {
    mapImg.src = isDark ? 'assets/story/europe_map.jpg' : 'assets/story/europe_map_light.jpg';
  }
...
function initSavedTheme() {
  try {
    const savedTheme = safeLocalStorage.getItem('eu_quiz_theme');
    if (savedTheme === 'dark') {
...
function resetGameStateForMode(mode) {
  gameMode = mode;
  teams = [];
  activeTeam = 0;
  singlePlayerScore = 0;
  ...
  if (typeof playedVisualQuestions !== 'undefined' && playedVisualQuestions.clear) {
    playedVisualQuestions.clear();
  }

  // Reset category screen UI elements
  ...
  cards.forEach(card => {
    card.classList.remove('card-disabled', 'disabled');
...
function showCategorySelect() {
  ...
  resetGameStateForMode(gameMode || 'campaign');
